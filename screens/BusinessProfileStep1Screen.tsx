...
export default function BusinessProfileStep1Screen({ navigation }: { navigation: any }) {
  ...
  const scrollContentStyle = useMemo(
    () => [
      ...
      { paddingBottom: 120 }, // fixed safe padding
    ],
    []
  );
  ...
  const lookupPostcode = async (postcode: string) => {
    ...
    try {
      ...
      Alert.alert('Postcode lookup failed');
    } finally {
      setIsLookingUpPostcode(false);
    }
  };
  ...
  const scrollContent = (
    <>
      <View style={styles.header}>
        <VectorBackButton
          onPress={() => safeGoBack(navigation, 'Consent')}
        />
        <Text style={styles.headerTitle}>Create Profile</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
          ...
          <View style={styles.profileImageContainer}>
            <TouchableOpacity
              ...
              style={[styles.imagePickerButton, isPickerLoading && { opacity: 0.6 }]}
            >
              {profileImage ? (
                <Image
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.inputWrapper}>
              <TextInput
                ...
                value={postalCode}
                onChangeText={setPostalCode}
                onBlur={() => lookupPostcode(postalCode)}
                style={styles.input}
                autoCapitalize="characters"
                placeholderTextColor="#9CA3AF"
              />
